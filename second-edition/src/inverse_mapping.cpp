...
int main()
{
  // names是存储名字的向量, 其下标和存储值形成天然的映射.
  vector<string> names;
  // IM是unordered_map<string, size_t>型映射, 其键类型是string, 值类型是size_t.
  ...
  {
    getline(infile, v, '\n');       // 接着读取v, 以'\n'作为分隔.
    auto c_iter_m = IM.find(u);       // 在IM中查找u是否存在映射值.
    if (c_iter_m == IM.cend())        // 若u在IM中没有对应的映射值.
    {
      names.push_back(u);             // 添加新名字.
      // 插入一对"键-值"(u, names.size() - 1).
  ...
  for (auto c_iter_m = IM.cbegin(); c_iter_m != IM.cend(); ++c_iter_m)
  {
    // c_iter_m->first和c_iter_m->second分别代表键和值.
    outfile << c_iter_m->first << '\t';
    outfile << c_iter_m->second << '\n';
  }
  outfile.close();
