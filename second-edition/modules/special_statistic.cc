export module special_statistic;
...
// 事实上, print_list函数可用print_all模块中的函数取代.
...
export void special_statistic(list<int>& L, int d)
{
  int maximum;
  input_preprocess(L, maximum);
  if (!L.empty())
    mean_of_larger(L, d, maximum);
}
